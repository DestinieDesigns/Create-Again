...
export const Mode1CompletionView: React.FC<Mode1CompletionViewProps> = ({
  ...
  const hasUnsavedChanges = !isSaved && (title.trim().length > 0 || photoDataUrl !== null || reflection !== null);
  ...
  const isSingleStart = session.promptHistory.length <= 1;
  const headline = isSingleStart
    ? MICROCOPY.completion.singleStart.headline
    : MICROCOPY.completion.fullSession.headline;
  const subtext = isSingleStart
    ? MICROCOPY.completion.singleStart.subtext
    : MICROCOPY.completion.fullSession.subtext;

  return (
    <div className="max-w-2xl mx-auto px-4 py-8 animate-fadeIn text-left">
      <div className="rounded-3xl bg-[#FCFAF6] border-3 border-[#2D2723] p-6 sm:p-10 paper-card subtle-shadow">
        ...
        <div className="text-center pb-6 border-b border-[#E8E0D5]">
          <div className="w-14 h-14 rounded-2xl bg-[#FFF2E6] text-[#E06D53] flex items-center justify-center mx-auto mb-3 shadow-xs">
            <Sparkles className="w-7 h-7" />
          </div>

          <div className="flex items-center justify-center gap-2 mb-1">
            <span className="text-xs font-bold uppercase tracking-wider text-[#8A7D71] font-mono-code">
              {isSingleStart ? 'First Mark Complete' : 'Session Complete'}
            </span>
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-[#2D2723] tracking-tight mt-1 font-sans">
            {headline}
          </h2>
          <p className="font-handwriting text-2xl text-[#63554A] font-bold mt-1">
            {subtext}
          </p>
          <p className="text-xs text-[#8A7D71] mt-1.5">
            {session.promptHistory.length} step{session.promptHistory.length === 1 ? '' : 's'} added to your sketchbook.
          </p>
        </div>

        {/* Reflection Section (Optional, no scoring or judgment) */}
        ...
        <div className="py-6 border-b border-[#E8E0D5] space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-[#8A7D71] mb-2 font-handwriting text-base">
              A private record of what you created, never analyzed or scored.
            </p>
            ...
            {photoDataUrl ? (
              ...
              >
                ...
                <div className="text-xs font-bold text-[#2D2723]">
                  Snap a photo with your camera or select a snapshot
                </div>
                <div className="text-[11px] text-[#8A7D71] mt-0.5">
                  Keep your real sketchbook page in your collection
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between gap-3 mb-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-[#8A7D71] font-mono-code bg-[#FAF7F2] border border-[#E8E0D5] px-2.5 py-0.5 rounded-lg">
                {session.promptHistory.length} step{session.promptHistory.length === 1 ? '' : 's'}
              </span>
          ...
          <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
            {session.promptHistory.map((item, index) => (
              <div
                ...
                className="p-3 rounded-2xl bg-[#FAF7F2] border border-[#E8E0D5] flex items-start gap-3"
              >
                <div className="w-6 h-6 rounded-lg bg-[#2D2723] text-[#FAF7F2] text-[11px] font-mono-code font-extrabold flex items-center justify-center shrink-0 mt-0.5">
                  {index + 1}
                </div>

                <div className="flex-1 min-w-0">
                  <span
                    className={`text-[9px] font-extrabold uppercase tracking-widest px-2 py-0.5 rounded-full border mb-1 inline-block ${getCategoryBadgeStyle(
                      item.category
                    )}`}
                  >
                    {item.category}
        ...
        <div className="pt-6 space-y-3">
          {!isSaved ? (
            <button
              onClick={handleSave}
              className="w-full py-4 rounded-xl bg-[#E06D53] hover:bg-[#CF5E45] text-white font-extrabold text-sm tracking-wide shadow-md transition-all active:scale-98 flex items-center justify-center gap-2 min-h-[48px]"
            >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 pt-1">
            <button
              ...
              className="w-full sm:flex-1 py-3 rounded-xl bg-[#2D2723] text-[#FAF7F2] font-extrabold text-xs hover:bg-[#433B35] transition-all flex items-center justify-center gap-1.5 min-h-[44px]"
            >
            ...
            <button
              ...
              className="w-full sm:flex-1 py-3 rounded-xl border border-[#D8CEBE] text-[#5C5046] font-bold text-xs hover:bg-[#EFE9DF] transition-all min-h-[44px]"
            >
              VIEW COLLECTION
            </button>
            ...
            <button
              ...
              className="w-full sm:w-auto px-4 py-3 rounded-xl text-xs font-semibold text-[#8A7D71] hover:text-[#2D2723] min-h-[44px]"
            >
              DONE
            </button>
          </div>
      ...
      {pendingAction && (
        ...
        >
          <div className="w-full max-w-sm bg-[#FCFAF6] rounded-3xl p-6 paper-card border-2 border-[#2D2723] text-center shadow-xl">
            <h3 className="text-lg sm:text-xl font-extrabold text-[#2D2723] tracking-tight">
              YOU HAVE UNSAVED CHANGES
            </h3>
            <p className="text-xs text-[#665A51] mt-2 leading-relaxed">
              You added details or a photo to this drawing. Would you like to save it to your collection before leaving?
            </p>
            ...
            <div className="mt-5 space-y-2">
              <button
                ...
                className="w-full py-3 rounded-xl bg-[#E06D53] hover:bg-[#CF5E45] text-white font-extrabold text-xs shadow-sm transition-all min-h-[44px]"
              >
                SAVE & CONTINUE
              </button>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-[#2D2723] text-white font-bold text-xs hover:bg-[#433B35] transition-all min-h-[44px]"
              >
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl border border-[#D8CEBE] text-[#6E6054] font-semibold text-xs hover:bg-[#EFE9DF] transition-all min-h-[40px]"
              >
                CANCEL
              </button>
            </div>
